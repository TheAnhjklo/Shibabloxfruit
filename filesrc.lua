...
spawn(function() 
    ...
    wait(1.5)
    require(game.ReplicatedStorage.Notification).new("<Color=Red>shibabloxfruit chào ae<Color=/>"):Display()
    wait(.5)
...
local Window = OrionLib:MakeWindow({Name = "shibabloxfruit", HidePremium = false, IntroText = "shibabloxfruit⚜️", SaveConfig = true, ConfigFolder = "NightHub"})
...
W:AddLabel("shibabloxfruit SCRIPT")
W:AddLabel("Discordshibabloxfruit")
local Section = W:AddSection({
    Name = "Status"
})
...
pcall(function()
    while wait() do
        if _G.AutoEliteHunter then
            if game:GetService("Players").LocalPlayer.PlayerGui.Main.Quest.Visible == true then
                OrionLib:MakeNotification({
                    Name = "shibabloxfruit(Noitification)",
